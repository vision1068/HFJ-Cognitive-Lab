...
export function WatchlistsPage() {
  ...
  const activeCompanies = active?.entries.map((e) => byTicker.get(e.ticker)).filter((c): c is NonNullable<typeof c> => Boolean(c)) ?? [];
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto">
      <PageHeader
        title="Watchlists"
        subtitle="Group PSX companies by strategy or sector and track their live performance."
        actions={
      ...
      {showCreate && (
        <div className="card p-4 mb-5 flex items-center gap-2">
          <input
            ...
            placeholder="Watchlist name (e.g. Banking Sector)"
          ...
          />
          <button onClick={handleCreate} className="bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium px-3.5 py-2 rounded-lg">Create</button>
      ...
      {active && (
        <div className="space-y-2.5">
          ...
          {!isLoading && activeCompanies.length === 0 && (
